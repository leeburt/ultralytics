#!/usr/bin/env python3
"""
随机抽取验证集100个样本，做bad case分析，生成可视化对比图。
"""
import cv2
import numpy as np
import torch
...
OUTPUT_ROOT = Path("/data-ssd/libo/p100/yolo_utils/dataset/external_inline/val_random_100_analysis")
NUM_SAMPLES = 100
SEED = 42
...
def main():
    ...
    for idx, (img_path, label_path) in enumerate(tqdm(selected, desc="分析bad case")):
    ...
    # 保存CSV
    csv_path = OUTPUT_ROOT / "bad_case_stats.csv"
    ...
    # 保存JSON
    json_path = OUTPUT_ROOT / "bad_case_stats.json"
